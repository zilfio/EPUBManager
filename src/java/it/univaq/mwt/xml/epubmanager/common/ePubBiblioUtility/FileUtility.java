...
public class FileUtility {
    ...
    public static void salvaFile(CommonsMultipartFile[] fileF, String path) {
        ...
        int tot = f.length;
        //Controllo che la cartella dove vado a salvare i file esiste o meno e in quest'ultimo caso la creo
        if (path.equals("") || path == "" || path == null) {
            ...
            //crea la cartella ePubBiblioUploaded
            current = path;
            current += "ePubBiblioUploaded";
            new File (current).mkdir();
        }
        //Ora scorro tutti i file e li salvo nella cartella
        for (int i = 0; i < tot; i++){
